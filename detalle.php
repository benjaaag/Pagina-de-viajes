...
$id_paquete = isset($GET['id']) ? intval(value: $_GET['id']) : 0;
...
if($id_paquete != null && $id_paquete > 0) {
    ...
    if (!$paquete) {
        ...
        SELECT * 
        FROM servicio JOIN paquete_servicio ON servicio.id_servicio = paquete_sevicio.id_servicio
        WHERE paquete_servicio.id_paquete = $id_paquete; "); 
    }
    $fechaLimite =  date_create(datetime: $paquete['f_limite']);
    $hoy = new DateTime();
